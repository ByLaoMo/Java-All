...
虽然可以不用配置文件，而在程序中实现配置，但这种方法在如今的系统开发中显然是不可取的，能采用配置文件的地方一定要用配置文件。
Log4J支持两 种格式的配置文件：XML格式和Java的Property格式。

### 操作步骤
```properties
# 设置日志输出的等级为debug,低于debug就不会输出
...
## 分析代码
与 Log4J 入门 中的 BasicConfigurator.configure(); 默认配置不同，这次采取的是制定的配置文件：
```java
